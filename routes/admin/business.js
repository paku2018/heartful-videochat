...
async function createBusiness(req){
    ...
    let sqlStr = "INSERT INTO t_business (name, code, zip, state, city, street, build_name, mobile, email, fax, unit, note) VALUES('" + name + "','" + code + "','" + zip + "','" + state + "','" + city + "','"
        + street + "','" + build_name + "','" + mobile + "','" + email + "','" + fax + "','" + unit + "','" + note + "')";
    console.log(sqlStr);
    let db = connection.makeDb();
    let  result = await db.query(sqlStr);
...
async function getAllBusinessList(type) {
...
async function getTotalBusiness() {
    let sqlStr = "SELECT COUNT(id) as cnt FROM t_business WHERE company_id = 0 AND status = 1";
    let db = connection.makeDb();
    let result = await db.query(sqlStr);
    return result[0].cnt;
}
...
module.exports = {
    ...
    getBusinessListApi: async function (req, res) {
        try {
            let totalCount = await getTotalBusiness();
    ...
    getAllBusinessListApi: async function (req, res) {
        try {
            let businessList = await getAllBusinessList(req.body.type);
            res.send(JSON.stringify({
                status: true,
                list: businessList
            }));
