...
async function createProduct (req){
    ...
    let sqlStr = "INSERT INTO t_product (business_id, company_id, name, type, type_detail, cost, fee, img, note) VALUES('" + business_id + "','" + company_id + "','" + name + "','" + type + "','" + type_detail + "','" +
        cost + "','" + fee + "','" + img + "','" + note + "')";
    console.log(sqlStr);
    let db = connection.makeDb();
    let  result = await db.query(sqlStr);
...
async function getProductList(req) {
    ...
    let sqlStr = "SELECT t_product.*, t_business.name as business_name " +
        " from t_product " +
        " left join t_business on t_product.business_id = t_business.id " +
        " where t_product.company_id = '" + company_id + "' AND t_product.status = 1 " +
...
async function getTotalProduct(req) {
    let company_id = req.user.id;
    let checkSqlStr = "SELECT COUNT(id) as cnt FROM t_product WHERE company_id = '" + company_id + "' AND status = 1";
    let db = connection.makeDb();
...
module.exports = {
    ...
    getProductListApi: async function (req, res) {
        try {
            let totalCount = await getTotalProduct(req);
